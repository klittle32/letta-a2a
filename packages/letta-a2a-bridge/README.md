...
The in-memory profile remains available. The opt-in [`DurableBinding` profile](../../docs/DURABLE_RECOVERY.md) persists official tasks, owner-scoped mappings, deduplication, execution fences, and publication repair. One controller owns a canonical local directory. Close stops admission and drains requests plus execution/persistence before owner release; incomplete shutdown remains explicit. Sent SDK interruption is uncertain, not confirmed cancellation. Recovery never automatically resends input. The operator guide covers readback, exact-evidence reconciliation, retention, and tested runtime limits.

## Delegation and push

`delegationPolicy` validates explicit `a2a_invoke` intent, asynchronous submission, authenticated delegate role, and canonical `x-letta-a2a-hop` (default maximum one). The application verifies the role/header, projects its decision onto `TrustedCaller.delegation`, and uses it to withhold or deny outbound tools. Supply the returned outbound headers through the client package's **host-owned** route policy, never model arguments. This is an application policy convention, not a new A2A field. The service retains its small legacy metadata adapter.
...
Extended cards remain explicitly disabled; the official client supports capable peers. The lab service consumes the packages and supports opt-in local single-owner recovery. REST/gRPC, arbitrary extensions/signatures, rich Letta input execution, distributed ownership, automatic in-flight replay, broad OS/backend certification, publication, and production deployment are not claimed. A custom durable TaskStore alone does not establish these recovery guarantees.
