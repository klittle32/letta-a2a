...
With the default volatile profile, restarting the bridge loses its in-memory A2A task records. Set `BRIDGE_DURABLE_DIRECTORY=/data/durable` for the opt-in [single-owner durable recovery profile](docs/DURABLE_RECOVERY.md): tasks, owner-scoped mappings, deduplication, and uncertain-execution fences survive. Push registrations remain process-local. Restarting the reference agent loses its in-memory tasks/context; restarting the webhook receiver loses its observation ledger.

Pre-convergence mapping entries had no caller identity. They remain on disk but are **not automatically assigned to a new authenticated caller**. New keys include binding/issuer/subject/tenant. No migration or deletion runs at startup, including when enabling durability. Saved conversation IDs do not prove active-task recovery: reconcile uncertain work before replay. Phase 5 deliberately preserves unresolved outcomes instead of resending input.
...
- Canceling an outer Letta task aborts outbound polling and requests cancellation of an accepted child and the local turn. The independent child can confirm cancellation; an SDK `interrupted` observation alone cannot confirm the outer backend stopped. Unknown outer execution fails with reconciliation required and stays quarantined. Tasks canceled while waiting on a conversation lock never submit input.
